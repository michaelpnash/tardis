  ...
  "the persistent event repository" - {
    "should not instantiate with a path that ends in a /" in {
      intercept[IllegalArgumentException] {
        new PersistentEventRepository(EventDirectory("foo/"))
      }
    }
    "can store and retrieve an event container across instantiations" in {
      ...
      target.mkdirs()
      target.delete()
      val repo = new PersistentEventRepository(EventDirectory(target.getPath))
      ...
      val repo2 = new PersistentEventRepository(EventDirectory(target.getPath))
